...
def search_show(library_key, query):
    """Search for a show in a library by listing all and filtering."""
    # Use /all endpoint which is more reliable than /search
    url = f"{PLEX_URL}/library/sections/{library_key}/all?X-Plex-Token={PLEX_TOKEN}"
    r = requests.get(url, headers={"Accept": "application/json"})
